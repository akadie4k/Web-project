    ...
    const fetchRequests = async () => {
      try {
        ...
        const { data: donorProfile, error: profileError } = await supabase
          .from("donor_profiles")
          .select(
            "donor_id, blood_type, rh_factor, province, is_ready, last_donate_date"
          )
          .eq("donor_id", userId)
          .single<DonorProfile>();
        ...
        if (donorProfile.last_donate_date) {
          const donatedAt = new Date(
            `${donorProfile.last_donate_date}T00:00:00`
          );

          const nextDonationDate = new Date(donatedAt);

          nextDonationDate.setDate(
            nextDonationDate.getDate() + 90
          );
          ...
          const daysRemaining = Math.max(
            0,
            Math.ceil(
              (nextDonationDate.getTime() - Date.now()) /
                millisecondsPerDay
            )
          );
        ...
        const matchedRequests = (requestData ?? []).filter(
          (request: any) => {
            ...
            {loading ? (
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-center shadow-sm">
                ...
                <p className="mt-1 text-sm text-slate-600">
                  ...
                  กรุณาเว้นระยะห่างการบริจาค 90 วัน
                  (เหลืออีก {coolingDownInfo.daysRemaining} วัน)
                </p>
              </div>
            ) : (
              <Suspense
